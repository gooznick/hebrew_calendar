...
def test_rav_ada_sun_year_moon_year():
    ...
    assert sun_year - moon_year == duration.duration("י", "כא", "קכא", "מח")
...
def test_rav_ada_sun_year_vs_19_cycle():
    ...
    assert sun_days_in_cycle == moon_days_in_cycle
...
def test_rav_ada_tkufa():
    ...
    assert rav_adas_tkufa == duration.duration("צא", "ז", "תקיט", "לא")
...
def test_rav_ada_tkufa_in_cycle_begin():
    ...
    for year in range(1, 100, 19):
        ...
        tkufa_according_to_molad_of_nissan = (
            molad.Months.molad(year, "ניסן") - duration.first_tkufa_diff_rav_ada
        )
        # convert to date
        date_of_molad_of_nissan = molad.Months.date_add_days(
            HDate(1, 1, 1), tkufa_according_to_molad_of_nissan.days
        )

        assert date_of_molad_of_nissan == rav_adas_tkufa_of_nissan[0]
        assert tkufa_according_to_molad_of_nissan.minutes == rav_adas_tkufa_of_nissan[2]
        assert tkufa_according_to_molad_of_nissan.hours == rav_adas_tkufa_of_nissan[1]
