...
def test_halacha_2():
    ...
    assert (v * 10000).copy().round_seconds().remove_circles() == angle(
        "קסט", "לא", "מ"
    )
...
def test_halacha_4():
    ...
    head_location = ecliptic_lat.EclipticLatitude.compute_head_location(the_day)
...
def test_halacha_19():
    ...
    is_south, lat = ecliptic_lat.EclipticLatitude.compute(the_day)
    assert is_south
...
def calc_moon_latitude(date: HDate):
    is_south, lat = ecliptic_lat.EclipticLatitude.compute(date)
