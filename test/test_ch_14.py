...
def test_halacha_2():
    assert (moon.mean_location_coefs["v"] * 10).remove_thirds() == angle(
        "קלא", "מה", 50
    )
    assert (moon.mean_location_coefs["v"] * 100).remove_circles() == angle(
        "רלז", "לח", "כג"
    )
    assert (moon.mean_location_coefs["v"] * 1000).remove_circles() == angle(
        "ריו", "כג", "נ"
    )
    assert (moon.mean_location_coefs["v"] * 10000).remove_circles() == angle(
        "ג", "נח", "כ"
    )
    assert (
        moon.mean_location_coefs["v"] * 29
    ).remove_circles().round_seconds() == angle("כב", 6, "נו")


@pytest.mark.xfail
def test_halacha_3a():
    assert (moon.mean_path_coefs["v"] * 10).remove_thirds() == angle("קל", "לט", 0)


@pytest.mark.xfail
def test_halacha_3b():
    assert (moon.mean_location_coefs["v"] * 354).remove_circles() == angle(
        "שמד", "כו", "מג"
    )


def test_halacha_3():
    assert (moon.mean_path_coefs["v"] * 100).remove_circles().remove_thirds() == angle(
        "רכו", "כט", "נג"
    )
    assert (moon.mean_path_coefs["v"] * 1000).remove_circles().remove_thirds() == angle(
        "קד", "נח", 50
    )
    assert (
        moon.mean_path_coefs["v"] * 10000
    ).remove_circles().remove_thirds() == angle("שכט", "מח", 20)
    assert (
        moon.mean_path_coefs["v"] * 29
    ).round_seconds().remove_circles().remove_thirds() == angle("יח", "נג", "ד")
